...
const CircularTrajectory = Trajectory{names, types, NamedTuple{names, trace_types}} where {names, types, trace_types<:Tuple{Vararg{<:CircularArrayBuffer}}}
...
function CircularTrajectory(; capacity, kwargs...)
    ...
    trajectories = merge(NamedTuple(), (name, CircularArrayBuffer{t}(s..., capacity)) for (name, (t, s)) in zip(names, types_and_sizes))
