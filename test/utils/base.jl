@testset "base" begin
    ...
    @testset "gae_returns" begin
        reward, values, γ, λ = [1.0], [2.0, 3.0], 0.5, 0.3
        @test gae_returns(reward, values, γ, λ) ≈ [0.5]

        reward, values, γ, λ = [1.0, 1.0], [1 , 2, 3] , 0.5, 0.3
        @test gae_returns(reward, values, γ, λ) ≈ [1.075, 0.5]

        reward, values, γ, λ = Float64[1, 2, 3], [1,2,3,4], 0.5, 0.3
        @test gae_returns(reward, values, γ, λ) ≈ [1.27, 1.8, 2]

        @test gae_returns(reward, values, γ, λ; terminal = [true false true]) ≈
              [0.0, 1.5, 0.0]

        # type stable
        reward, values, γ, λ = [1, 2, 3], [1,2,3,4], 0.5f0, 0.5f0
        @test eltype(gae_returns(reward, values, γ, λ)) == Float32

        # 2D
        reward, values, γ, λ = reshape(1:9, 3, 3), reshape(1:12, 4, 3), 0.5, 0.3
        # for 2d rewards, the keyword argument of `dim` must be either `1` or `2`
        @test_throws MethodError gae_returns(reward, values, γ, λ)
        @test gae_returns(reward, values, γ, λ; dims = 1) ≈
              [1.27 2.4425 3.615; 1.8 2.95 4.1; 2.0 3.0 4.0]

        values = reshape(1:12, 3, 4)
        @test gae_returns(reward, values, γ, λ; dims = 2) ≈
              [2.6375 4.25 5.0; 3.22375 4.825 5.5; 3.81 5.4 6.0]

        reward, values, γ, λ = reshape(1:9, 3, 3), reshape(1:12, 4, 3), 0.5, 0.3
        terminal = [false true false; true false true; false true false;]

        @test gae_returns(reward, values, γ, λ; dims = 1, terminal = terminal) ≈
